...
def sidebar():
    ...
    with st.sidebar:
        uploaded_file = st.sidebar.file_uploader(
            label="Upload your csv or excel file (200 MB Max).", type=["csv", "xlsx"]
        )
...
def mainContent():
    ...
    if df is not None:
        ...
        if opt == "Line Chart":
            label = f"Line Chart for {filename} is"
            st.header(label)
            st.line_chart(dataToVisualize)

        elif opt == "Bar Chart":
            label = f"Bar Chart for {filename} is"
            st.header(label)
            st.bar_chart(dataToVisualize)
        elif opt == "Pie Chart":
            label = f"Pie Chart for {filename} is"
            st.header(label)
            ...
            if len(dataToVisualize) != 0:
                plt.pie(
                    x, labels=selectedData, autopct="%.5f%%"
                )  # %).f%% means no of digit show after decimal
