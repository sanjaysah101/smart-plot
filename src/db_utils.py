...
def store_dataset(dataset_name, dataset_df, text_column_for_embedding=None):
    ...
    if text_column_for_embedding and text_column_for_embedding in dataset_df.columns:
        ...
        dataset_df[text_column_for_embedding] = dataset_df[text_column_for_embedding].astype(str).fillna('')
        ...
        print(f"Warning: Text column '{text_column_for_embedding}' not found or not specified. Storing data without embeddings.")
    ...
    if "embedding" in dataset_df.columns:
        try:
            create_vector_index(collection, "embedding", 768) # 768 is dimension for text-embedding-004
            print(f"INFO: Embeddings generated for collection '{dataset_name}'. "
...
def create_vector_index(collection, field_name, vector_dimension, index_name="vector_index"):
    ...
    try:
        ...
        print(f"INFO: Vector search index '{index_name}' on field '{field_name}' needs to be created in MongoDB Atlas.")
        ...
        print(f"Error during vector index creation check/attempt for field '{field_name}': {e}")
...
def vector_search(collection_name, query_text, index_field="embedding", num_results=5, text_field_to_return=None):
    ...
    if not query_vector or all(v == 0.0 for v in query_vector): # Check if embedding failed or is zero vector
    ...
    try:
        ...
        print("Please ensure that a vector search index named 'vector_index' exists on the collection "
